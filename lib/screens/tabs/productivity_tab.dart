...
class _ProductivityTabState extends State<ProductivityTab> {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildMporChart(mporData, mporAvg, dateLabels),

          const SizedBox(height: 24),

          /// Chart 2: Tasks Completed per Labor Hour
          _buildTasksChart(tasksData, dateLabels),

          const SizedBox(height: 24),

          /// Chart 3: Revenue per Labor Hour
          _buildRevenueChart(revenueData, dateLabels, minIndex, maxIndex),
        ],
      ),
    );
  }

  Widget _buildMporChart(List<double> mporData, List<double> mporAvg, List<String> dateLabels) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
  ...
  Widget _buildRevenueChart(List<double> revenueData, List<String> dateLabels, int minIndex, int maxIndex) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
                  ...
                  lineTouchData: LineTouchData(
                    touchTooltipData: LineTouchTooltipData(
                      getTooltipItems: (touchedSpots) {
                        return touchedSpots.map<LineTooltipItem?>((spot) {
                          String label = '';
                          if (spot.spotIndex == minIndex) label = 'Min';
                          if (spot.spotIndex == maxIndex) label = 'Max';
                          return LineTooltipItem(
